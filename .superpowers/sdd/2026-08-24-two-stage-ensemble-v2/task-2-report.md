...
Final blocker fixes: model contract now asserts best iteration `7`, positive/fixed refit constructor value, and exact inner/eval/refit row counts `4/2/6`; bootstrap contract now demonstrates that changing user cluster assignment with the same seed changes the confidence interval while retaining equal-fold point-delta checks.
