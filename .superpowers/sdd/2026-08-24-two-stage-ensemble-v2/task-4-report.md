...
- Focused command: `python -m unittest tests.test_models tests.test_artifacts tests.test_inference -v`
  - Latest run: **16/16 tests passed**, including strict CatBoost objective, positive-only refit, EBM default overhead/ndarray rounds, true bundle persistence/provenance and 91-feature inference regressions.
- Full v2 command: `python -m unittest discover -s tests -v`
  - Latest run: **37 passed, 1 failed**. The only failure is the expected `test_required_distributions_are_installed`: the optional `interpret` distribution is absent. `models.py` does not import InterpretML at package import time; `fit_ebm_classifier` imports it only when EBM fitting is requested.
...
- CatBoost requires strict inner end-to-end RMSLE inputs, retains winning `ParameterSampler` parameters, and refits a fresh estimator. Plain/Ordered flags, explicit loss/eval/bootstrap settings, native missing values, `allow_writing_files=False`, deterministic seed 42, and at most 10 estimator threads are enforced; Ordered trials are capped at three and require measured Plain governance metrics.
- EBM has lazy import, native `estimate_mem(X_outer_train, y_outer_train, data_multiplier=1)` support, explicit temporal bags, ndarray best-iteration aggregation by maximum positive stage/bag, measured same-settings toy-fit RSS overhead, fixed-round outer refit settings, and a strict estimated-memory-plus-overhead gate. Missing/invalid overhead or temporal-bags `TypeError` is a structured rejection.
